...
│   ├── raw/
│   │   ├
│   │   └── malware_webshell/  악성 웹쉘 파일 (하위 폴더 포함)
│   └── preprocessed/          전처리가 완료된 .csv 파일
└── ...

실행:
    프로젝트 루트에서
    python src/preprocessing/make_dataset_webshell.py

출력:
    data/preprocessed/dataset_webshell.csv
"""
...
# 프로젝트 루트 경로 (project/src/preprocessing/make_dataset_webshell.py 기준)
PROJECT_ROOT = Path(__file__).resolve().parents[2]

RAW_DATA_DIR = PROJECT_ROOT / "data/raw"
BENIGN_WEBSHELL_DIR = RAW_DATA_DIR / "benign_webshell"
MALWARE_WEBSHELL_DIR = RAW_DATA_DIR / "malware_webshell"
DEFAULT_OUTPUT = PROJECT_ROOT / "data/preprocessed/dataset_webshell.csv"
...
def make_dataset_webshell(
    output_csv: str | Path = DEFAULT_OUTPUT,
) -> pd.DataFrame:
    """benign_webshell/malware_webshell 파일들을 preprocess_many()로 처리하여 CSV를 생성한다."""
    ...
    print(f"  benign_webshell : {len(benign_files)}개")
    print(f"  malware_webshell: {len(malware_files)}개")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="raw/benign_webshell 및 malware_webshell 내 파일을 탐색하여 Dataset CSV로 변환"
    )
    ...
    parser.add_argument(
        ...
        help="출력 CSV 경로 (기본값: data/preprocessed/dataset_webshell.csv)",
    )

    args = parser.parse_args()
    make_dataset_webshell(args.output)
