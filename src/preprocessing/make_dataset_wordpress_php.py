...
│   ├── raw/
│   │   ├
│   │   └── malware_wordpress_php/  악성 웹쉘 파일 (하위 폴더 포함)
│   └── preprocessed/          전처리가 완료된 .csv 파일
└── ...

실행:
    프로젝트 루트에서
    python src/preprocessing/make_dataset_wordpress_php.py

출력:
    data/preprocessed/dataset_wordpress_php.csv
"""
...
# 프로젝트 루트 경로 (project/src/preprocessing/make_dataset_wordpress_php.py 기준)
PROJECT_ROOT = Path(__file__).resolve().parents[2]

RAW_DATA_DIR = PROJECT_ROOT / "data/raw"
BENIGN_WEBSHELL_DIR = RAW_DATA_DIR / "benign_wordpress_php"
MALWARE_WEBSHELL_DIR = RAW_DATA_DIR / "malware_wordpress_php"
DEFAULT_OUTPUT = PROJECT_ROOT / "data/preprocessed/dataset_wordpress_php.csv"
...
def make_dataset_wordpress_php(
    output_csv: str | Path = DEFAULT_OUTPUT,
) -> pd.DataFrame:
    """benign_wordpress_php/malware_wordpress_php 파일들을 preprocess_many()로 처리하여 CSV를 생성한다."""
    ...
    print(f"  benign_wordpress_php : {len(benign_files)}개")
    print(f"  malware_wordpress_php: {len(malware_files)}개")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="raw/benign_wordpress_php 및 malware_wordpress_php 내 파일을 탐색하여 Dataset CSV로 변환"
    )
    ...
    parser.add_argument(
        ...
        help="출력 CSV 경로 (기본값: data/preprocessed/dataset_wordpress_php.csv)",
    )

    args = parser.parse_args()
    make_dataset_wordpress_php(args.output)
